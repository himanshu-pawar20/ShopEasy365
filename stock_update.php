...
<body>
  ...
  <div class="form-group">
    ...
    <input type="number" class="form-control" id="exampleInputphoneno" name="up_stock">
  </div>
...
<?php
$temp_sp3=$_SESSION['special_id'];
$table_name="{$temp_sp3}product";
if(isset($_POST['up_submit'])){
  $new_up_stock=$_POST['up_stock'];
  $new_date=date('Y-m-d');
  $up_id1=$_SESSION['pid2'];
  $p_result=mysqli_query($con,"SELECT * FROM $table_name WHERE PID = '" .$up_id1. "'");
  $u_row=mysqli_fetch_assoc($p_result);
  $last_stock=$u_row['PSTOCK'];
  $comlete_stock=$new_up_stock+$last_stock;
  $new_up=mysqli_query($con,"UPDATE $table_name SET PSTOCK = '".$comlete_stock."',PLAST_STOCK_UPDATE='".$new_date."' WHERE PID = '" .$up_id1. "'");
