...
if(isset($_POST['up_submit'])){
  ...
  $sql6="SELECT* FROM $table_name10 WHERE BID='$up_id1'";
  $result6=mysqli_query($con,$sql6);
  $row6=mysqli_fetch_assoc($result6);
  $rborrowing=$row6['BBORROWING_AMOUNT'];
  $return=$rborrowing-$new_amount;
  $rgain_total=$new_amount+$row6['BGAIN_AMOUNT'];
  $sq=mysqli_query($con,"UPDATE $table_name10 SET `BGAIN_AMOUNT`='$rgain_total',`BBORROWING_AMOUNT`='$return',`last_update_date`='$new_date' WHERE BID='$up_id1'");
  if($sq){
